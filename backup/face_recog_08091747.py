    ...
    def get_frame(self):
        ...
        for face_encoding in self.face_encodings:
            ...
            if min_value < 0.6:
                ...
                self.send_recog_log(frame,log,False)
        ...
        if len(self.face_names) == 0:   #아무도 감지 안되면
            ...
            if (self.start_time - self.end_time).seconds > 5:          #10초 이상 부재 일때
                ...
                self.send_recog_log(frame,log,True)
        else:
            self.counting=False
            self.absence = False #아 왔다

    
    def send_recog_log(self,frame,log,absence):
        ret, jpg = cv2.imencode('.jpg', frame)
        frame_byte = jpg.tobytes()
        if absence==True and datetime.now().second % 5 != 0:
            return
        else:
            data = dict()
            data["type"] = log
