...
from WatchU.models import Professor, TestRoom, Student, Log
from WatchU import db
from WatchU.views import auth_views
...
def edit_list():
    ...
    for t in professor.test_room_set:
        test_room_list.append((t.id, t.state, t.title, str(t.start_date)[:16], str(t.end_date)[10:16]))
...
def watching_before(test_room_id):
    ...
    for s in test_room.student_set:
        student_list.append(s.id)
    return render_template('test_room/watching_before.html', test_room_id=test_room_id, title=test_room.title, start_date=str(test_room.start_date)[:16], end_date=str(test_room.end_date)[10:16], student_list=student_list)
...
def start(test_room_id):
    ...
    for s in test_room.student_set:
        ...
        log_buffer[s.id] = list()
    global_images[test_room_id] = images

    # 부정행위 로그 버퍼 생성
    watching_log_buffer[test_room_id] = set()
    # 학생 세부정보 페이지 부정행위 로그 버퍼 생성
    watching_detail_log_buffer[test_room_id] = log_buffer
...
def log(test_room_id, student_id):
    ...
    watching_log_buffer[test_room_id].add(student_id)  # 부정행위 감지 학생 저장
    watching_detail_log_buffer[test_room_id][student_id].append({  # 부정행위 감지 내용 저장
        "type": type,
        "date": str(date),
        "image": image
    })
...
def watching_detail_log_ajax(test_room_id, student_id):
    ...
    if len(watching_detail_log_buffer[test_room_id][student_id]) != 0:
        ...
        watching_detail_log_buffer[test_room_id][student_id].clear()
        watching_log_buffer[test_room_id].remove(student_id)
        return jsonify(json_list)
    else:
        return jsonify(json_list)
